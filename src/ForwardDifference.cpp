...
ForwardDifference::ForwardDifference(double dt) : HeatEquationSolver(dt) {}
...
void ForwardDifference::solve(PDE::HeatEquationProblem &heatEq) {
  ...
  //       - Can we move computation of sigma and the stencil somewhere else so
  ...
  Eigen::MatrixXd wij = Eigen::Map<Eigen::MatrixXd>(
      ((heatEq.getDomainPtr())->getDomainAsVectorPtr())->data(), spatialDivs,
      spatialDivs);
  wij.resize(spatialDivs * spatialDivs, 1);

  Eigen::SparseMatrix<double> stencil(spatialDivs * spatialDivs,
                                      spatialDivs * spatialDivs);
  Eigen::SparseMatrix<double> bcs(spatialDivs * spatialDivs,
                                  spatialDivs * spatialDivs);

  std::vector<Eigen::Triplet<double>> coeffTriplets;
  for (int i = 1; i < spatialDivs - 1; ++i) {
    for (int j = 1; j < spatialDivs - 1; ++j) {
      coeffTriplets.push_back(Eigen::Triplet<double>(
          i * spatialDivs + j, i * spatialDivs + j, 1 - (4 * sigma)));
      coeffTriplets.push_back(Eigen::Triplet<double>(
          i * spatialDivs + j, i * spatialDivs + j + 1, sigma));
      coeffTriplets.push_back(Eigen::Triplet<double>(
          i * spatialDivs + j, i * spatialDivs + j - 1, sigma));
      coeffTriplets.push_back(Eigen::Triplet<double>(
          i * spatialDivs + j, i * spatialDivs + j + spatialDivs, sigma));
      coeffTriplets.push_back(Eigen::Triplet<double>(
          i * spatialDivs + j, i * spatialDivs + j - spatialDivs, sigma));
    }
  }
  ...
  for (int i = 0; i < spatialDivs; ++i) {
    for (int j = 0; j < spatialDivs; ++j) {
      ...
      if (i == 0) {
        bcsTriplets.push_back(Eigen::Triplet<double>(
            i * spatialDivs + j, i * spatialDivs + j + spatialDivs, bc));
      }
      if (i == spatialDivs - 1)
        bcsTriplets.push_back(Eigen::Triplet<double>(
            i * spatialDivs + j, i * spatialDivs + j - spatialDivs, bc));
      ...
      if (j == 0) {
        bcsTriplets.push_back(Eigen::Triplet<double>(
            i * spatialDivs + j, i * spatialDivs + j + 1, bc));
      }
      if (j == spatialDivs - 1)
        bcsTriplets.push_back(Eigen::Triplet<double>(
            i * spatialDivs + j, i * spatialDivs + j - 1, bc));
